...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embeddings = self.word_embeddings(
            captions
        )  # embeddings new shape : (batch_size, captions_length - 1, embed_size)
        embeddings = torch.cat(
            (features.unsqueeze(1), embeddings), dim=1
        )  # embeddings new shape : (batch_size, caption_length, embed_size)
        out, self.hidden = self.rnn(
            embeddings, self.hidden
        )  # out shape : (batch_size, caption_length, hidden_size)
